...
interface FlightSearchProps {
  onSearch: (params: any, type: string) => void;
  isLoading: boolean;
}

export default function FlightSearch({ onSearch, isLoading }: FlightSearchProps) {
  const [searchType, setSearchType] = useState<'flight' | 'airport' | 'airline'>('flight');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (searchType === 'flight') {
      params.flights = query;
    } else if (searchType === 'airport') {
      params.airports = query.toUpperCase();
    } else if (searchType === 'airline') {
      params.operating_as = query;
    }
  ...
  return (
    <div className="space-y-4">
      <Tabs value={searchType} onValueChange={(v: any) => setSearchType(v)} className="w-full">
