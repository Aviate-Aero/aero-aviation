...
function parseTafPeriods(raw: string): TafPeriod[] {
  ...
  const lines = raw.split(/\s+(?=FM|TEMPO|BECMG)/)
...
function getTafFlightCategory(visText: string | null, ceilingFt: number | null): string {
  let vis = 10
...
function TafPeriodCard({ period, isFirst = false }: TafPeriodCardProps) {
  const flightCat = getTafFlightCategory(period.visibility?.text ?? null, period.clouds.ceiling)
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-4 text-sm">
        <div>
          ...
          <span className="ml-2 text-zinc-200 font-mono">{period.wind.text || '—'}</span>
        </div>
        <div>
          <span className="text-zinc-500">Visibility:</span>
          <span className="ml-2 text-zinc-200">{period.visibility?.text || '—'}</span>
        </div>
        <div>
      ...
      {period.clouds.ceiling && (
        <div className="mt-2 text-xs text-zinc-500">Ceiling: {period.clouds.ceiling} ft</div>
      )}
    </div>
  )
}
  ...
  const fetchTafs = useCallback(async () => {
    ...
    try {
      const [r1, r2] = await Promise.all([
        fetch(`/api/avwx?type=taf&mode=single&icao=${dep}`),
        fetch(`/api/avwx?type=taf&mode=single&icao=${arr}`),
      ])
    ...
    return (
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h4 className="text-lg font-medium text-zinc-200">Forecast Analysis</h4>
          {time && (
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h2 className="text-3xl font-light text-white">Terminal Aerodrome Forecast</h2>
          <p className="text-zinc-400">Aviation weather forecasts for departure and arrival</p>
        </div>
      </div>
      ...
      <div className="bg-zinc-900/80 backdrop-blur-sm border border-zinc-800 rounded-2xl p-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="space-y-3">
            <Label htmlFor="taf-dep" className="text-zinc-300 font-medium flex items-center gap-2">
          ...
          <div className="space-y-3">
            <Label htmlFor="taf-arr" className="text-zinc-300 font-medium flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 transition-all duration-300 hover:border-zinc-700">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-lg font-medium text-white">Departure TAF</h3>
          ...
          {tafDep && tafDep !== '—' && renderTafAnalysis(tafDep)}
        </div>
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 transition-all duration-300 hover:border-zinc-700">
          ...
          {tafArr && tafArr !== '—' && renderTafAnalysis(tafArr)}
        </div>
      </div>
    </motion.div>
  )
