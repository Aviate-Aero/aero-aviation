...
const FR24_COUNT_URL =
  "https://fr24api.flightradar24.com/api/live/flight-positions/count";
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (typeof data?.record_count !== "number") {
      return NextResponse.json(
        {
          error: "Invalid FlightRadar24 count response format.",
          data,
        },
        { status: 502 }
      );
    ...
    return NextResponse.json({
      ...
      endpoint: "live/flight-positions/count",
      retrieved_at: new Date().toISOString(),
    });
  } catch (error) {
    return NextResponse.json(
      {
        error: "Internal server error while fetching live flight position count.",
...
function buildFr24Params(searchParams: URLSearchParams) {
  ...
  for (const [key, value] of searchParams.entries()) {
    ...
    if (PASSTHROUGH_PARAMS.has(key) && trimmedValue) {
      params.set(key, normalizeCommaList(trimmedValue));
    }
  }
