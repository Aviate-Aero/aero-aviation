...
export async function GET() {
  try {
    ...
    const endpoint = `${baseUrl}/delays/faa`;
    ...
    if (!response.ok) {
      console.error(
        "Skylink global FAA delays API failed:",
        JSON.stringify(
          {
    ...
    console.error("Skylink global FAA delays route error:", error);

    return NextResponse.json<ErrorResponse>(
      {
        message: "Internal server error while fetching global FAA delay data.",
