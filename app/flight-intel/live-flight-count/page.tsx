...
export default function LiveFlightCountPage() {
  ...
  const fetchCount = async (event?: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/live-flight-positions-count?${params.toString()}`
      );
      const data = await parseApiResponse(response);

      if (!response.ok) {
        setErrorMessage(
          getErrorMessage(data) || "Failed to fetch live flight position count."
        );
        return;
      }

      if (!isCountResult(data)) {
        setErrorMessage("Unexpected count response from the server.");
        return;
      }
      ...
      setErrorMessage(
        error instanceof Error
          ? error.message
          : "Failed to fetch live flight position count."
      );
    } finally {
      setLoading(false);
    }
    ...
    <main className="relative min-h-screen overflow-hidden bg-black text-white mt-40">
      <section className="relative z-20 mx-auto w-full max-w-7xl px-4 pb-20 pt-32 sm:px-6 lg:px-8">
        <div className="mb-8 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-3 max-w-3xl text-base text-zinc-400">
              Count live aircraft positions by region, route, airport, operator,
              aircraft type, altitude, speed, category, and data source.
            </p>
          </div>
        ...
        <form onSubmit={fetchCount} className="space-y-6">
          <Card className="border-zinc-800/50 bg-zinc-900/50 backdrop-blur-xl rounded-2xl overflow-hidden">
            <CardHeader className="border-b border-zinc-800/50 pb-4">
              ...
              <CardDescription className="text-zinc-400">
                Start broad for a total live count, or narrow the count with
                one or more operational filters.
              </CardDescription>
            </CardHeader>
            ...
            <CardContent className="space-y-6 pt-6">
              ...
              <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
                <FilterField
                  ...
                  hint="North, south, west, east. Use a preset or enter custom coordinates."
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
            <FilterPanel
              ...
              description="Target specific flights, callsigns, registrations, or operators."
              icon={<Hash className="h-5 w-5 text-sky-400" />}
            >
              <FilterField
                label="Flight Numbers"
                value={filters.flights}
                placeholder="CA4515,UA1742"
                hint="Max 15 comma-separated values."
                onChange={(value) => updateFilter("flights", value)}
              />
              <FilterField
                label="Callsigns"
                value={filters.callsigns}
                placeholder="WJA329,WSW102"
                hint="Operational callsigns."
                onChange={(value) => updateFilter("callsigns", value)}
              />
              <FilterField
                label="Registrations"
                value={filters.registrations}
                placeholder="D-AFAM,EC-MQM"
                hint="Aircraft registration numbers."
                onChange={(value) => updateFilter("registrations", value)}
              />
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <FilterField
                  label="Operating As"
                  value={filters.operating_as}
                  placeholder="SAS,ART"
                  hint="Airline callsign ICAO."
                  onChange={(value) => updateFilter("operating_as", value)}
                />
                <FilterField
                  label="Painted As"
                  value={filters.painted_as}
                  placeholder="SAS,ART"
                  hint="Aircraft livery ICAO."
                  onChange={(value) => updateFilter("painted_as", value)}
                />
              </div>
            </FilterPanel>
            ...
            <FilterPanel
              ...
              description="Count positions by airport pair, aircraft type, airspace, or transponder state."
              icon={<Route className="h-5 w-5 text-emerald-400" />}
            >
              <FilterField
                label="Routes"
                value={filters.routes}
                placeholder="SE-US,ESSA-JFK"
                hint="Airport or country pairs."
                onChange={(value) => updateFilter("routes", value)}
              />
              <FilterField
                label="Aircraft Types"
                value={filters.aircraft}
                placeholder="B38M,B738"
                hint="ICAO aircraft type codes."
                onChange={(value) => updateFilter("aircraft", value)}
              />
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <FilterField
                  label="Squawks"
                  value={filters.squawks}
                  placeholder="6135,7070"
                  hint="Hex squawk codes."
                  onChange={(value) => updateFilter("squawks", value)}
                />
                <FilterField
                  label="Airspaces"
                  value={filters.airspaces}
                  placeholder="ESAA,LFFF"
                  hint="Lower or upper FIR codes."
                  onChange={(value) => updateFilter("airspaces", value)}
                />
              </div>
            </FilterPanel>
          </div>
          ...
          <Card className="border-zinc-800/50 bg-zinc-900/50 backdrop-blur-xl rounded-2xl overflow-hidden">
            <CardHeader className="border-b border-zinc-800/50 pb-4">
              ...
              <CardDescription className="text-zinc-400">
                Refine the count by aircraft class, source, speed, and altitude.
              </CardDescription>
            </CardHeader>
            ...
            <CardContent className="space-y-5 pt-6">
              <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
                <FilterField
                  label="Altitude Ranges"
                  value={filters.altitude_ranges}
                  placeholder="0-3000,5000-7000"
                  hint="Barometric altitude above mean sea level, in feet."
                  icon={<Layers className="h-4 w-4 text-purple-400" />}
                  onChange={(value) => updateFilter("altitude_ranges", value)}
                />
                <FilterField
                  label="Ground Speed"
                  value={filters.gspeed}
                  placeholder="120-140 or 80"
                  hint="Single value or range in knots."
                  icon={<Gauge className="h-4 w-4 text-emerald-400" />}
                  onChange={(value) => updateFilter("gspeed", value)}
                />
              </div>
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex flex-wrap gap-2">
              {activeFilters.length > 0 ? (
                ...
                <span className="text-sm text-zinc-500">
                  No filters selected. The count will cover all visible live
                  positions available to the endpoint.
                </span>
              )}
            </div>
            ...
            <div className="flex gap-3">
              ...
              >
                ...
                {loading ? "Counting..." : "Get Live Count"}
              </Button>
            </div>
          </div>
        </form>
              ...
              <div className="flex items-start gap-3 text-red-300">
                ...
                <div>
                  <p className="font-medium">
                    Failed to fetch live position count
                  </p>
          ...
          <section className="mt-6 space-y-6">
                ...
                <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
                  ...
                  <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
                    ...
                    <ResultFact
                      ...
                      label="Retrieved"
                      value={formatDateTime(result.retrieved_at)}
                    />
                  </div>
                </div>
              </CardContent>
            ...
            <Card className="border-zinc-800/50 bg-zinc-900/50 backdrop-blur-xl rounded-2xl overflow-hidden">
              <CardHeader className="border-b border-zinc-800/50 pb-4">
                ...
                <CardDescription className="text-zinc-400">
                  The count above is based on these FlightRadar24 query filters.
                </CardDescription>
              </CardHeader>
              ...
              <CardContent className="pt-6">
                {Object.keys(result.filters).length > 0 ? (
                  ...
                  <div className="rounded-xl border border-zinc-800/50 bg-black/20 p-5 text-zinc-400">
                    No filters were applied. This is the broad live position
                    count returned by the endpoint.
                  </div>
                )}
              </CardContent>
            </Card>
        ...
        {!result && !errorMessage && !loading && (
          <Card className="mt-6 border-zinc-800/50 bg-zinc-900/50 backdrop-blur-xl rounded-2xl">
            <CardContent className="flex flex-col items-center justify-center py-16 text-center">
              <Radar className="mb-5 h-20 w-20 text-zinc-600" />
              <h2 className="mb-2 text-2xl font-light text-zinc-300">
                Ready for a Live Count
              </h2>
              <p className="max-w-2xl text-zinc-500">
                Choose a preset area or enter operational filters, then fetch
                the current number of live aircraft positions.
              </p>
            </CardContent>
          </Card>
        )}
