...
export default function FaaDelaysPage() {
  const [icao, setIcao] = useState("KJFK");

  const [result, setResult] = useState<FaaDelaysResponse | null>(null);
  const [loading, setLoading] = useState(false);
  const [errorMessage, setErrorMessage] = useState("");

  const searchFaaDelays = async () => {
    try {
      setLoading(true);
      setErrorMessage("");
      setResult(null);
      ...
      const response = await fetch(
        `/api/skylink/delays/airport-delays?icao=${encodeURIComponent(cleanIcao)}`
      );
      ...
      if (!response.ok) {
        setErrorMessage(
          data?.message ||
            ...
            "Failed to fetch FAA delay data."
        );
        return;
      }

      setResult(normalizeFaaDelaysResponse(data));
    } catch (error) {
      console.error(error);
        ...
        <Card className="border-zinc-800/50 bg-zinc-900/50 backdrop-blur-xl rounded-2xl overflow-hidden">
          <CardHeader className="pb-4 border-b border-zinc-800/50">
            ...
            <CardDescription className="text-zinc-400">
              Check FAA delay alerts for an airport, including ground delays,
              ground stops, closures, and airspace flow programs.
            </CardDescription>
          </CardHeader>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="flex items-end">
                ...
                >
                  ...
                  {loading ? "Checking..." : "Check Airport Delays"}
                </Button>
              </div>
            </div>
          ...
          <Card className="border-red-500/30 bg-red-500/10 backdrop-blur-sm rounded-2xl">
            <CardContent className="pt-6">
              <div className="flex items-start gap-3 text-red-300">
                ...
                <div>
                  <p className="font-medium">Failed to fetch FAA delays</p>
            ...
            {totalAlerts(result) > 0 ? (
              <div className="space-y-6">
                {groundDelays.length > 0 && (
                  <AlertSection
                    title="Ground Delays"
                    description="FAA-reported ground delay programs affecting the airport."
                ...
                {groundStops.length > 0 && (
                  <AlertSection
                    title="Ground Stops"
                    description="FAA-reported ground stop programs affecting the airport."
                ...
                {closures.length > 0 && (
                  <AlertSection
                    ...
                    description="FAA-reported airport closure information."
              ...
              <Card className="border-zinc-800/50 bg-zinc-900/50 backdrop-blur-xl rounded-2xl">
                <CardContent className="flex flex-col items-center justify-center py-20 text-center">
                  ...
                  <p className="text-zinc-500 max-w-md">
                    ...
                    program alerts were returned for this airport.
                  </p>
                </CardContent>
              </Card>
            )}
...
function normalizeFaaDelaysResponse(data: unknown): FaaDelaysResponse {
  ...
  const value = data as Partial<FaaDelaysResponse>;
